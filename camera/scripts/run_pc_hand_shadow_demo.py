...
def _video_device_index(source: int | str) -> int | None:
...
def open_capture(source: int | str, capture_factory=cv2.VideoCapture):
    index = _video_device_index(source)
...
def _draw_overlay(frame, result):
    ...
    if getattr(result, "shadow_mask", None) is not None:
        ...
        output[mask] = (output[mask] * 0.7 + np.array([80, 40, 0]) * 0.3).astype(np.uint8)
    if getattr(result, "shadow_contour", None) is not None:
        cv2.drawContours(output, [result.shadow_contour], -1, (255, 255, 0), 2)
