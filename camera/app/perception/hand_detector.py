...
class _MediaPipeBackend:
    ...
    def detect(self, frame: np.ndarray) -> list[HandDetection]:
        ...
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            palm_center = _estimate_palm_center(points)
            detections.append(
                HandDetection(
                    bbox=bbox,
                    center=center,
...
def _estimate_palm_center(points: list[tuple[int, int]]) -> tuple[int, int] | None:
    if len(points) < 18:
        return None
    palm_indices = [0, 5, 9, 13, 17]
    selected = [points[index] for index in palm_indices]
    return (
        int(round(sum(point[0] for point in selected) / len(selected))),
        int(round(sum(point[1] for point in selected) / len(selected))),
    )
...
class MediaPipeHandDetector:
    ...
    def detect(self, frame: np.ndarray) -> HandDetectorResult:
        ...
        hands = []
        for hand in self.backend.detect(frame):
            if hand.palm_center is None:
                hand = HandDetection(
                    ...
                    palm_center=_estimate_palm_center(hand.landmarks),
                )
            hands.append(hand)
        return HandDetectorResult(True, hands, warning=self._warning)
